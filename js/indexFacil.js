...
function cargarPregunta(index){
    objetoPregunta = baseDePreguntasFacil[index]
...
async function seleccionarOpcion(index) {
    ...
    INDEX_PREGUNTA++;
    if (INDEX_PREGUNTA >= baseDePreguntasFacil.length){
        INDEX_PREGUNTA = 0;
        await Swal.fire({
            title:"Juego terminado",
            html:`<span class="subtitulo-ventana">Tu puntaje es de: ${puntaje}/${baseDePreguntasFacil.length}</span>`,
...
modo.addEventListener("click", function(){
    ...
    if(valor=="true"){
        body.classList.add("bkfacildk");
        body.classList.remove("bkfacil");
    }else{
        body.classList.remove("bkfacildk");
        body.classList.add("bkfacil");
    }
})
...
if (valor=="true") {
    ...
    body.classList.remove("blfacildk");
    body.classList.add("bkfacil");
}
