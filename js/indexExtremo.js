...
function cargarPregunta(index){
    objetoPregunta = baseDePreguntasExtremo[index]
...
async function seleccionarOpcion(index) {
    ...
    INDEX_PREGUNTA++;
    if (INDEX_PREGUNTA >= baseDePreguntasExtremo.length){
        INDEX_PREGUNTA = 0;
        await Swal.fire({
            title:"Juego terminado",
            html:`<span class="subtitulo-ventana">Tu puntaje es de: ${puntaje}/${baseDePreguntasExtremo.length}</span>`,
...
modo.addEventListener("click", function(){
    ...
    if(valor=="true"){
        body.classList.add("bkextremodk");
        body.classList.remove("bkextremo");
    }else{
        body.classList.remove("bkextremodk");
        body.classList.add("bkextremo");
    }
})
...
if (valor=="true") {
    ...
    body.classList.remove("blextremo");
    body.classList.add("bkextremo");
}
